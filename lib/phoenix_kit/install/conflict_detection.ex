defmodule PhoenixKit.Install.ConflictDetection do
  ...
  def perform_comprehensive_analysis(igniter, opts \\ []) do
    ...
    with {:ok, dependency_analysis} <- run_dependency_analysis(igniter),
         ...
         {:ok, migration_strategy} <- maybe_generate_migration_strategy(
           dependency_analysis, config_analysis, code_analysis, generate_strategy, opts),
         {:ok, overall_assessment} <- generate_overall_assessment(
           dependency_analysis, config_analysis, code_analysis, migration_strategy) do
      ...
      comprehensive_analysis = %{
        ...
        recommendations: compile_all_recommendations(dependency_analysis, config_analysis, code_analysis),
  ...
  def quick_conflict_check(_igniter) do
    ...
    with {:ok, basic_deps} <- check_basic_auth_dependencies(),
      ...
      quick_assessment = %{
        ...
        estimated_conflict_level: estimate_quick_conflict_level(basic_deps, basic_configs, basic_user_schemas),
        recommendation: generate_quick_recommendation(basic_deps, basic_configs, basic_user_schemas),
        should_run_full_analysis: should_run_full_analysis?(basic_deps, basic_configs, basic_user_schemas)
      }
      
      Logger.info("⚡ Quick check complete - estimated conflict level: #{quick_assessment.estimated_conflict_level}")
      {:ok, quick_assessment}
    else
      error ->
  ...
  def assess_coexistence_feasibility(comprehensive_analysis) do
    ...
    coexistence_assessment = %{
      ...
      required_changes: compile_required_changes_for_coexistence(dependency_conflicts, config_conflicts, code_conflicts),
    ...
    coexistence_assessment = %{coexistence_assessment |
      overall_feasible: coexistence_assessment.dependency_feasibility and 
  ...
  defp run_code_analysis(igniter, scan_test_files, max_files) do
    ...
    case CodeAnalyzer.analyze_authentication_code(igniter, 
           scan_test_files: scan_test_files, max_files: max_files) do
      {:ok, result} ->
  ...
  defp maybe_generate_migration_strategy(dependency_analysis, config_analysis, code_analysis, true, opts) do
    Logger.debug("Generating migration strategy...")
    case MigrationAdvisor.generate_migration_strategy(dependency_analysis, config_analysis, code_analysis, opts) do
      {:ok, strategy} ->
  ...
  defp generate_overall_assessment(dependency_analysis, config_analysis, code_analysis, migration_strategy) do
    total_conflicts = count_total_conflicts(dependency_analysis, config_analysis, code_analysis)
    critical_conflicts = count_critical_conflicts(dependency_analysis, config_analysis, code_analysis)
    
    overall_assessment = %{
      total_conflicts: total_conflicts,
      critical_conflicts: critical_conflicts,
      overall_risk_level: determine_overall_risk_level(critical_conflicts, total_conflicts),
      installation_complexity: determine_installation_complexity(dependency_analysis, code_analysis),
      ...
      requires_manual_intervention: requires_manual_intervention?(dependency_analysis, config_analysis, code_analysis),
      auto_resolvable_conflicts: count_auto_resolvable_conflicts(dependency_analysis, config_analysis, code_analysis),
      blocking_issues: identify_blocking_issues(dependency_analysis, config_analysis, code_analysis)
    }
    ...
    try do
      case File.read("mix.exs") do
        {:ok, content} ->
          ...
          found_deps = Enum.filter(auth_deps, fn dep ->
  ...
  defp check_basic_auth_configs do
    ...
    try do
      auth_configs = Enum.flat_map(config_files, fn file ->
  ...
  defp check_basic_user_schemas do
    try do
      case Path.wildcard("lib/**/*user*.ex") do
        [] -> {:ok, []}
        files -> 
          user_schemas = Enum.filter(files, fn file ->
            case File.read(file) do
  ...
  defp compile_required_changes_for_coexistence(dep_conflicts, config_conflicts, _code_conflicts) do
    ...
    changes = changes ++ Enum.flat_map(dep_conflicts, fn conflict ->
    ...
    changes = changes ++ Enum.flat_map(config_conflicts, fn conflict ->
  ...
  defp log_comprehensive_analysis_summary(analysis) do
    ...
    Logger.info("   Manual intervention: #{analysis.overall_assessment.requires_manual_intervention}")
