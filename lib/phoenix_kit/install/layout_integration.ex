defmodule PhoenixKit.Install.LayoutIntegration do
  ...
  def perform_full_integration(igniter, opts \\ []) do
    ...
    with {:ok, layout_analysis} <- run_layout_detection(igniter),
         {:ok, integration_strategy} <-
           determine_integration_strategy(layout_analysis, layout_preference),
         {:ok, updated_igniter, config_result} <-
           perform_auto_configuration(igniter, integration_strategy, opts),
         {:ok, final_igniter, enhancement_result} <-
           maybe_enhance_layouts(updated_igniter, integration_strategy, enhance_layouts),
         {:ok, complete_igniter, fallback_result} <-
           maybe_create_fallbacks(final_igniter, integration_strategy, create_fallbacks) do
      ...
      integration_result = %{
        ...
        recommendations:
          compile_integration_recommendations(layout_analysis, integration_strategy),
  ...
  def quick_layout_check(_igniter) do
    ...
    with {:ok, basic_layouts} <- check_basic_layout_structure(),
      ...
      Logger.info(
        "⚡ Quick check complete - integration complexity: #{quick_assessment.integration_complexity}"
      )
      ...
      error ->
        Logger.warning(
          "⚡ Quick check failed, recommend running full integration: #{inspect(error)}"
        )
  ...
  defp determine_integration_strategy(_layout_analysis, strategy)
       when strategy in [:existing, :phoenix_kit] do
    case strategy do
  ...
  defp check_basic_layout_structure do
    try do
      layout_files =
        Path.wildcard("lib/*/web/components/layouts/*.html.heex") ++
  ...
  defp check_existing_layout_configs do
    try do
      ...
      has_phoenix_kit_config =
        Enum.any?(config_files, fn file ->
          case File.read(file) do
            {:ok, content} ->
              String.contains?(content, "phoenix_kit") and String.contains?(content, "layout")
  ...
  defp estimate_integration_complexity(basic_layouts, existing_configs) do
    ...
    complexity_score =
      complexity_score + if existing_configs.has_phoenix_kit_config, do: 1, else: 0
