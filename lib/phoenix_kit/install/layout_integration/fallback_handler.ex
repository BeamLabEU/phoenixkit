defmodule PhoenixKit.Install.LayoutIntegration.FallbackHandler do
  ...
  def create_fallback_layouts(igniter, integration_strategy, opts \\ []) do
    ...
    with {:ok, fallback_strategy} <- determine_fallback_strategy(integration_strategy),
         ...
         {:ok, updated_igniter, created_fallbacks} <-
           execute_fallback_creation(igniter, creation_plan, target_dir, custom_templates),
         {:ok, configured_igniter, config_updates} <-
           configure_fallback_system(updated_igniter, created_fallbacks, fallback_strategy) do
  ...
  def update_fallback_layouts(igniter, fallback_updates) do
    ...
    with {:ok, existing_fallbacks} <- discover_existing_fallbacks(igniter),
         {:ok, updated_igniter, update_results} <-
           apply_fallback_updates(igniter, existing_fallbacks, fallback_updates) do
      {:ok, updated_igniter,
       %{
  ...
  defp determine_fallback_strategy(integration_strategy) do
    ...
    fallback_strategy =
      Map.get(strategy_mapping, integration_strategy, :use_phoenix_kit_fallbacks)
  ...
  defp determine_target_directory(igniter, nil) do
    ...
    target_dir =
      Enum.find(possible_directories, fn dir ->
  ...
  defp execute_fallback_creation(igniter, creation_plan, target_dir, custom_templates) do
    ...
    created_fallbacks =
      creation_plan.planned_fallbacks
      |> Enum.map(fn layout_type ->
        create_single_fallback(
          igniter,
          layout_type,
          target_dir,
          custom_templates,
          creation_plan.enhancement_level
        )
      end)
