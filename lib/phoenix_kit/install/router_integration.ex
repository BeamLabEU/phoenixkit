defmodule PhoenixKit.Install.RouterIntegration do
  ...
  def perform_full_integration(igniter, opts \\ []) do
    ...
    with {:ok, router_info} <- ASTAnalyzer.find_and_analyze_router(igniter),
         {:ok, igniter, skip_result} <-
           maybe_skip_if_exists(igniter, router_info, skip_if_exists),
         {:ok, igniter, conflict_resolution} <-
           resolve_conflicts_if_needed(igniter, router_info, prefix, auto_resolve),
      ...
      error ->
        {:error,
         %{
           ...
           feasible: false,
           recommendations: [
             "Ensure your Phoenix application has a properly structured router.ex file"
           ]
         }}
    end
  end
