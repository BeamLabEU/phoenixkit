defmodule PhoenixKit.Install.RouterIntegration.ASTAnalyzer do
  ...
  def find_and_analyze_router(igniter) do
    with {:ok, router_path} <- find_router_file(igniter),
      ...
      Logger.info("✅ Router found and analyzed: #{inspect(router_module)}")
      {:ok, router_info}
    else
      {:error, reason} = error ->
  ...
  def find_router_file(igniter) do
    ...
    # Сначала попробуем стандартные паттерны для конкретного приложения
    specific_patterns = [
      "lib/#{app_name}_web/router.ex",
      "lib/#{app_name}/router.ex"
    ]
    
    all_patterns = specific_patterns ++ @router_patterns
    
    case find_first_existing_file(all_patterns) do
      {:ok, path} -> 
        Logger.debug("Router file found: #{path}")
        {:ok, path}
      
      :error ->
        {:error, {:router_not_found, %{
          searched_patterns: all_patterns,
          suggestion: "Ensure your Phoenix application has a router.ex file in standard location"
        }}}
    end
  end
