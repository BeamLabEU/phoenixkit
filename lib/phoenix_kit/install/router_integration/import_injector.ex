defmodule PhoenixKit.Install.RouterIntegration.ImportInjector do
  ...
  defp add_import_after_use(zipper) do
    ...
    case Igniter.Code.Common.add_code(zipper, import_ast, placement: :after) do
      ...
      unexpected ->
        Logger.error("Unexpected return from add_code in add_import_after_use: #{inspect(unexpected)}")
        zipper
    end
  end
  ...
  defp add_import_after_defmodule(zipper) do
    ...
    Logger.warning("Import addition not implemented in fallback - import may need to be added manually")
    zipper
  end
