defmodule PhoenixKit.Install.RouterIntegrationTest do
  ...
  describe "perform_full_integration/2" do
    ...
    test "respects custom prefix option", %{igniter: igniter} do
      ...
      assert {:ok, _updated_igniter, result} = RouterIntegration.perform_full_integration(igniter, opts)
    ...
    test "skips integration when routes already exist", %{test_dir: test_dir} do
      ...
      assert {:ok, _igniter, integration_result} = result or 
    ...
    test "handles validation errors gracefully", %{igniter: igniter} do
      ...
      with_mock(Validator, [:passthrough], [
        ...
        assert {:ok, _updated_igniter, result} = RouterIntegration.perform_full_integration(igniter)
  ...
  describe "check_integration_feasibility/2" do
    setup do
      test_dir = System.tmp_dir!() |> Path.join("phoenix_kit_feasibility_test_#{:rand.uniform(10000)}")
      File.mkdir_p!(test_dir)
    ...
    test "handles missing router gracefully", %{} do
      ...
      assert {:error, error_result} = RouterIntegration.check_integration_feasibility(empty_igniter)
  ...
  describe "rollback_integration/2" do
    setup do
      test_dir = System.tmp_dir!() |> Path.join("phoenix_kit_rollback_test_#{:rand.uniform(10000)}")
      File.mkdir_p!(test_dir)
  ...
  describe "generate_integration_report/2" do
    ...
    test "generates comprehensive integration report", %{igniter: igniter} do
      ...
      assert report.integration_status in [:not_integrated, :partially_integrated, :fully_integrated, :error]
    end
  end
