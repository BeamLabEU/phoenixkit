...
defmodule IntegrationTestValidator do
  ...
  defp validate_test_file_structure do
    ...
    {missing, existing} =
      Enum.split_with(test_files, fn file ->
  ...
  defp validate_integration_completeness do
    ...
    coverage_results =
      Enum.map(coverage_areas, fn {area, file, keywords} ->
    ...
    total_expected =
      coverage_results |> Enum.map(fn {_, _, expected} -> expected end) |> Enum.sum()
    ...
    if coverage_percentage >= 80 do
      {"Integration Coverage", :ok,
       "#{coverage_percentage}% coverage (#{total_covered}/#{total_expected})"}
    else
      {"Integration Coverage", :warning,
       "#{coverage_percentage}% coverage (#{total_covered}/#{total_expected}) - could be better"}
    end
  end
